...
int estudiante_InicializarCurso(eCurso array[], int size)                                    //cambiar estudiante
{
    int retorno=-1;
    int i;
    if(array!= NULL && size>0)
...
int estudiante_buscarEmptyCurso(eCurso array[], int size, int* posicion)                    //cambiar estudiante
{
    int retorno=-1;
    int i;
    if(array!= NULL && size>=0 && posicion!=NULL)
...
int estudiante_buscarIDCurso(eCurso array[], int size, int valorBuscado, int posicion)                    //cambiar estudiante
{
    int retorno=-1;
    ...
    if(array!= NULL && size>=0)
    {
            ...
            {
                if (array[posicion].isEmpty==0)           //cambiar campo ID
                {
                    retorno=0;
                }
             }
...
int estudiante_alta(eEstudiante array[], int size, int* contadorID)                          //cambiar estudiante
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        else
        {
            ...
            //utn_getTexto("getTexto\n: ","\nError",1,TEXT_SIZE,2,array[posicion].nombre);                 //mensaje + cambiar campo varLongString
...
int estudiante_altaCurso(eCurso array[], int size, eEstudiante arrayEstudiante[], int sizeEst, int* contadorIDcurso)                          //cambiar estudiante
{
    ...
    if(array!=NULL && size>0 && contadorIDcurso!=NULL)
    {
        if(estudiante_buscarEmptyCurso(array,size,&posicion)==-1)                          //cambiar estudiante
        {
            printf("\nNo hay lugares vacios");
        }
        else
        {
            (*contadorIDcurso)++;
            array[posicion].idCurso=*contadorIDcurso;                                                       //campo ID
            utn_getUnsignedInt("\nIngrese id alumno: ","\nError ingreso id alumno",1,sizeof(int),18,100,2,&legajoEstudiante);
            if(estudiante_buscarID(arrayEstudiante,sizeEst,legajoEstudiante,&legajoEncontrado)==0)
            {
            ...
            utn_getUnsignedInt("\nIngrese id materia: ","\nError ingreso materia",1,sizeof(int),1,100,2,&array[posicion].idMateria);
            utn_getUnsignedInt("\nIngrese nota: ","\nError ingreso nota",1,sizeof(int),1,100,2,&array[posicion].nota);
            array[posicion].isEmpty=0;
            //printf("\n Posicion: %d\n ID: %d\n varInt: %d\n varFloat: %f\n varString: %s\n varLongString: %s",
            //       posicion, array[posicion].idEstudiante,array[posicion].varInt,array[posicion].varFloat,array[posicion].varString,array[posicion].varLongString);
            retorno=0;
...
int estudiante_baja(eEstudiante array[], int sizeArray)                                      //cambiar estudiante
{
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        else
        {
            ...
            array[posicion].idEstudiante=0;                                                                   //cambiar campo id
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d\n edad: %d\n nombre: %s",
                       array[i].idEstudiante,array[i].edad,array[i].nombre);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
...
int estudiante_listarMaterias(eMateria array[], int size)                      //cambiar estudiante
{
    int retorno=-1;
    int i;
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n IDMateria: %d nombre: %s ",
                       array[i].idMateria,array[i].descripcionMateria);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
...
int estudiante_listarCursos(eCurso array[], int size)                      //cambiar estudiante
{
    int retorno=-1;
    int i;
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n IDCurso: %d\n legajo: %d\n materia: %d\n nota: %d",
                       array[i].idCurso,array[i].idEstudiante, array[i].idMateria, array[i].nota);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
}
int estudiante_buscarNombrePorId(eEstudiante arrayE[], int TAM,int id, char* nombre)
{
    int retorno=-1;
    int i;
    ...
    for (i=0;i<TAM;i++)
    {
       if (arrayE[i].idEstudiante==id)
       {
           strcpy(nombre,arrayE[i].nombre);
           retorno=0;
       }
...
int materia_buscarNombrePorId(eMateria arrayE[], int TAM,int id, char* nombre)
{
    int retorno=-1;
    int i;
    ...
    for (i=0;i<TAM;i++)
    {
       if (arrayE[i].idMateria==id)
       {
           strcpy(nombre,arrayE[i].descripcionMateria);
           retorno=0;
       }
...
int materia_buscarIdPorNombre(eMateria arrayE[], int TAM,char* nombre,int *id)
{
    int retorno=-1;
    int i;
    ...
    for (i=0;i<TAM;i++)
    {
       if (strcmp(nombre,arrayE[i].descripcionMateria)==0)
       {
           *id=arrayE[i].idMateria;
           retorno=0;
       }
    }
    return retorno;
}
int materia_buscarMayorNota(eCurso arrayCur[],int sizeCur,int idmateria,int* maxNota,int* idEstudiante)
{
    int i;
    int retorno=-1;
    ...
    for(i=0;i<sizeCur;i++)
    {
        if(arrayCur[i].idMateria==idmateria)
        {
            if(i==0)
            {
                *maxNota=arrayCur[i].nota;
                *idEstudiante=arrayCur[i].idEstudiante;
                retorno=0;
            }
            else
            {
                if(*maxNota<arrayCur[i].nota)
                {
                  *maxNota=arrayCur[i].nota;
                  *idEstudiante=arrayCur[i].idEstudiante;
                  retorno=0;
